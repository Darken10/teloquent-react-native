...
class BelongsToMany extends Relation_1.Relation {
    ...
    convertToIds(value) {
        ...
        return ids.filter((v) => typeof v === 'string' || typeof v === 'number');
    }
    /**
    ...
    async loadForCollection(collection, relationName) {
        ...
        const rawParentKeys = collection.pluck(this.parentKey);
        const parentKeys = rawParentKeys.filter((v) => typeof v === 'string' || typeof v === 'number');
        ...
        results.each((model) => {
            ...
            model.setAttribute('pivot', pivotAttributes);
        ...
        collection.each((model) => {
            ...
            model.setRelation(baseName, new Collection_1.Collection(relationModels));
        });
    }
    /**
